...
const App: React.FC = () => {
  ...
  const handleContactRequest = (id: number) => {
    setMembers(prev => prev.map(m => 
      m.id === id ? { ...m, contactRequested: true, requestedAt: new Date().toLocaleString() } : m
    ));
  };
      ...
      <div className="min-h-screen flex flex-col font-sans selection:bg-[#800000] selection:text-white bg-white">
        ...
        <main className="flex-grow">
          <Routes>
            ...
            <Route path="/mappillai" element={<Mappillai members={members} />} />
            <Route path="/penn" element={<Penn members={members} />} />
            <Route path="/profile/:id" element={<ProfileDetail members={members} onContactRequest={handleContactRequest} />} />
            ...
            <Route path="/admin/dashboard" element={<AdminDashboard members={members} setMembers={setMembers} />} />
            <Route path="/admin/add-member" element={<AdminAddMember onAdd={addMember} />} />
          </Routes>
        </main>
        <Footer />
      </div>
