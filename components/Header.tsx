...
export const Header: React.FC = () => {
  ...
  const lineVariants = {
    ...
    open: (i: number) => {
      ...
      if (i === 1)
        return { opacity: 0, width: "0px", transition: { duration: 0.1 } };
      if (i === 2)
        return {
          rotate: -45,
          y: 0,
        ...
        <nav className="hidden lg:flex items-center gap-6 xl:gap-8">
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
              className={`text-sm font-bold uppercase tracking-wider transition-colors text-slate-500 hover:text-slate-900`}
            >
          ...
          <div className="flex items-center gap-3 border-l border-slate-200 pl-6 ml-2">
            <Link
              to="/login"
              className="text-sm font-black uppercase tracking-wider text-[#800000] hover:opacity-80 transition-opacity mr-2"
            >
              {t("nav_login")}
            </Link>
            <Link
              to="/admin/login"
              className="w-8 h-8 flex items-center justify-center rounded-full bg-slate-50 text-slate-400 hover:text-[#800000] hover:bg-slate-100 transition-all"
              title="Admin Login"
            >
              <svg
                className="w-4 h-4"
          ...
          >
                ...
                {navLinks.map((link, idx) => (
                  ...
                  >
                    <Link
                      ...
                      className={`block py-3 text-3xl font-black uppercase tracking-tight font-tamil border-b border-slate-50 text-slate-800 hover:text-[#800000] transition-colors`}
                    >
                      {link.name}
                    </Link>
                  </motion.div>
              ...
              >
                ...
                <div className="w-full text-center space-y-4">
                  <Link
                    to="/login"
                    onClick={() => setIsOpen(false)}
                    className="block py-3 text-xl font-black uppercase tracking-tight text-[#800000] hover:opacity-80 transition-colors"
                  >
                    {t("nav_login")}
                  </Link>
                  <Link
                    to="/admin/login"
                    onClick={() => setIsOpen(false)}
                    className="block py-3 text-xl font-bold uppercase tracking-tight text-slate-800 hover:text-[#800000] transition-colors"
                  >
                    {t("admin_login")}
                  </Link>
                </div>
            ...
            <div className="p-10 bg-slate-50/50 flex flex-col items-center gap-4">
              <div className="flex items-center gap-2 opacity-40">
                <div className="w-6 h-6 rounded bg-[#800000] flex items-center justify-center text-[#D4AF37] font-bold text-[10px]">
                  A
                </div>
              ...
              <p className="text-[9px] text-slate-300 font-medium uppercase tracking-widest">
                © 2024 AGAMUDAIYAR Traditional Services
              </p>
            </div>
          </motion.div>
        )}
